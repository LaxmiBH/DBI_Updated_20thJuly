Feature: Shift-Break as Start Time Only Required Functionality

  Scenario: Start Time Only Required
    ...
    And In "Name:" textbox User Enter "STRequired"
    And In "Abbreviation:" textbox User Enter "req"
    And In "Code:" textbox User Enter "ST-30"
    And In Duration User Enter "M:" as "30"
    And User clicks "Required:" checkbox
    When User Scroll down popup upto Create New Break button
    And In "Suggested Early Start:" textbox User Enter "9:00"
    And User selects AM for "Suggested Early Start:"
    And In "Suggested Late Start:" textbox User Enter "10:30"
    And User selects AM for "Suggested Late Start:"
    ...
    When User enters "STRequired" characters in Search bar
    Then Break "STRequired" should be displayed
    ...
    And Select "STRequired" break from dropdown
    ...
    And User clicks on "Punch Clock" option from left on Remote Payroll Settings
    When User clicks "Manage Punch Clock Breaks" button on Remote Payroll Settings
    Then Delete the "STRequired" break from Manage Punch Clock Breaks
